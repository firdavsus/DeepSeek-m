...
def prepare_data_char(path, extra_tokens=None, val_ratio=0.01):
    with open(path, "r", encoding="utf-8") as f:
        text = f.read()

    # Take first 25% for training (example)
    train_text = text[:len(text)//4]
...
if __name__ == "__main__":
    ...
    data, data_val, stoi, itos = prepare_data_char("data_text/combined_text.txt", extra_tokens=config.special_tokens)

    model_simple = LLM().to(device)
    # model_simple = torch.compile(model_simple_not_comp, mode="reduce-overhead", fullgraph=False)
    ...
    train(model_simple, train_loader, val_loader, stoi, itos, epochs=5, accumulation_steps=4)
